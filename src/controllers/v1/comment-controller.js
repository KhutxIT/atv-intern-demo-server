        ...
        .then((comment) => {
          comment.populate('author', (err) => {
            ...
            User.find({ username: { $in: uniqueUsernames } })
              .select('_id')
              .then((userIds) => {
                const removedUserid = arrayRemove(userIds, req.user._id);
            ...
            Promise.all([user, notification, commentLike])
              .then((values) => {
                sendAddCommentNotification(req, values);
                const data = {
                  _id: comment._id,
                  author: [
                    {
...
const getRepliesForComment = (req, res) => {
  let query;
...
const likeCommentReply = (req, res) => {
...
const getCommentReplyLikes = (req, res) => {
